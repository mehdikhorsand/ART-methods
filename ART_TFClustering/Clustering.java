...
public class Clustering {
    ...
    public static ArrayList<Cluster> clustering_analysis(ArrayList<TrisectionFrequencyConversion> originalData) {
        int k = get_suitable_k(originalData.size());
        ArrayList<Cluster> clustering = new ArrayList<>();
        for(int i=0; i<k; i++){
            TrisectionFrequencyConversion initial_testcase = originalData.get(MyRandom.getInt(0, originalData.size()-1));
            clustering.add(new Cluster(initial_testcase, i));
            originalData.remove(initial_testcase);
        }
        for(TrisectionFrequencyConversion tc_ft : originalData)
        ...
        for(int i=0; i<Settings.maximum_iteration && change; i++){
            ...
            for(TrisectionFrequencyConversion tc_ft : originalData) {
                ...
                for(int j=0; j<k; j++) {
                    double distance = TrisectionFrequencyConversion.get_distance(tc_ft, clustering.get(j).centroid);
                    if(distance < minDis){
                        minDis = distance;
                        nearestCluster = j;
                    }
