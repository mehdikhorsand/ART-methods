...
public class Clustering {
    ...
    public static ArrayList<Cluster> clustering_analysis(ArrayList<WaveletTransform> originalData) {
        int k = get_suitable_k(originalData.size());
        ArrayList<Cluster> clustering = new ArrayList<>();
        for(int i=0; i<k; i++){
            WaveletTransform initial_testcase = originalData.get(MyRandom.getInt(0, originalData.size()-1));
            clustering.add(new Cluster(initial_testcase, i));
            originalData.remove(initial_testcase);
        }
        for(WaveletTransform tc_ft : originalData)
        ...
        for(int i=0; i<Settings.maximum_iteration && change; i++){
            ...
            for(WaveletTransform tc_ft : originalData) {
                ...
                for(int j=0; j<k; j++) {
                    double distance = WaveletTransform.get_distance(tc_ft, clustering.get(j).centroid);
                    if(distance < minDis){
                        minDis = distance;
                        nearestCluster = j;
                    }
